...
#[derive(Debug)]
pub struct DHHashMap<K, V>(Vec<Option<RawEntry<K, V>>>);

impl<K, V> DHHashMap<K, V> {
...
impl<K, V> MapMut<K, V> for DHHashMap<K, V> {
...
impl<'a, K, V> MapUtil<'a, K, V> for DHHashMap<K, V> {
...
impl<K, V> IntoMapUtil<K, V> for DHHashMap<K, V> {
...
impl<K, V> IntoIterator for DHHashMap<K, V> {
    type Item = (K, V);
...
impl<K, V> MapSize for DHHashMap<K, V> {
    fn len(&self) -> usize {
        todo!()
    }
}

impl<K, V> Default for DHHashMap<K, V> {
    fn default() -> Self {
        Self::new(DEFAULT_CAPACITY)
    }
}

impl<K: Hash + Ord, V> From<Vec<(K, V)>> for DHHashMap<K, V> {
